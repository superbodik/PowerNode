...
<style>
  ...
  #player iframe { position: absolute; width: 1px; height: 1px; opacity: 0; pointer-events: none; border: 0; }
  ...
  <script>
    ...
    function buildPlayer(descriptor) {
      if (descriptor.kind === 'youtube') {
        ...
        f.src = 'https://www.youtube.com/embed/' + encodeURIComponent(descriptor.id) + '?autoplay=1&controls=0';
        return f;
      }
      if (descriptor.kind === 'soundcloud') {
        ...
        f2.src = 'https://w.soundcloud.com/player/?url=' + encodeURIComponent(descriptor.url) + '&auto_play=true&visual=false&show_artwork=false';
        return f2;
      }
